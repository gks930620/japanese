...
| 규칙 | 내용 | 왜 |
|---|---|---|
| 병합 키 | **표기(word) + 읽기 둘 다** 같을 때만 한 행. **읽기 = 일본어 `kana` · 영어 `ipa`** | 읽기가 다르면 다른 단어다. 두 값의 **쌍**을 키로 쓰므로 문자열을 이어붙일 때 생기는 경계 모호성이 없고, 읽기 `null`도 그대로 키가 된다 |
...
| `ipa`·`koApprox` | 그룹 대표 행의 값 — 그런데 **키가 읽기를 포함하므로 그룹의 모든 행이 같은 값**이다 | 대표를 고르는 규칙과 무관하게 한 행의 발음은 하나로 확정된다 |
...
| 검색 대상 | 표기·`kana`(행 단위) + 뜻(sense 단위) 중 **하나라도** 맞으면 통과 | |
| `KANA` 정렬 | `kana`가 없으면 **표기**로 정렬. **가타카나를 히라가나 자리로 옮겨** 섞어 정렬 | 사전에서 찾는 감각과 맞추기 위해 |

#### 읽기는 언어마다 다른 컬럼에 있다 (2026-09-23 정정 — 08 A-8)

병합 키의 둘째 값은 **"읽기"라는 개념**이고, 그것이 담긴 컬럼이 언어마다 다르다.

| 과정 | 읽기 | 반대쪽 컬럼 |
|---|---|---|
| 일본어 | `kana` | `ipa`는 **전량 NULL**(계약 J-8) |
| 영어 | `ipa` | `kana`는 **전량 NULL**(설계/06 §11-3) |

두 컬럼은 **서로 배타적**이므로 읽기는 행에서 `kana ?? ipa`로 정해진다 — 요청 언어를 보지 않는다.
그래서 일본어 동작은 오늘과 **완전히 같다**(일본어 행의 `ipa`가 NULL이라 폴백이 일어나지 않는다).

⚠️ **`ipa`가 읽기인 것은 병합 키에서만이다.** 위 표의 `검색 대상`·`KANA 정렬`은 그대로 `kana`를 쓴다 —
영어 자료실을 `ipa`로 정렬하면 `/ə/`로 시작하는 단어가 앞에 몰려 **알파벳순이 아니게** 되고,
학습자가 자료실에서 기대하는 것은 사전순이다. 영어는 `kana`가 없으므로 지금처럼 **표기(word)** 로 정렬된다.
...
- **어휘 병합 키의 '읽기'는 `ipa`다**(§3-7 "읽기는 언어마다 다른 컬럼에 있다" · 08 A-8).
  표기가 같아도 발음이 다르면 **별개 행**이다 — `close` `/kloʊs/`(가까운, E1)와 `/kloʊz/`(닫다, E2)는 두 행,
  `leave` `/liːv/`(E1·E2)는 한 행 + `senses` 2개. 고정 테스트는
  `EnglishVocabularyMergeContractIntegrationTest`다.
