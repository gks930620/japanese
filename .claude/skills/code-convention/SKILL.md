...
- 예외 ②: **기동 시 검증(startup guard)처럼 컨트롤러로 도달할 수 없는 구성 요소**도 단위테스트를 허용한다.
  근거: 이런 코드는 애초에 요청을 받지 않고 **애플리케이션 기동을 막는 것이 동작 자체**라, 통합테스트로 검증하면
  컨텍스트가 뜨지 않아 테스트가 성립하지 않는다. 검증이 없으면 규칙(예: 운영에서 H2 콘솔·편집 모드 기동 거부)이
  아무도 지키지 않는 주석으로 남는다. 예: `RailwayDeploymentValidatorTest`.
