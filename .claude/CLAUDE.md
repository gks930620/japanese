...
- **콘텐츠 구조 (확정)**:
  ...
  - 실력 진단(시작점 추천)은 **이미 동작한다** — `/diagnosis`. 레벨을 직접 골라 한 판(6문항)씩 치고, 통과든 미달이든 결과 화면에서 멈춘다. 규칙은 `설계/09 §3`. (2026-09-21 정정 — "추후 추가 예정"으로 남아 있었다.)
...
| `05_디자인시스템.md` | **Lets** 4원칙, 그라디언트 예산, 컴포넌트 재사용 (킷을 Halo → Lets로 교체 완료 — 2026-09-10) |
...
| `_보일러플레이트_잔재/` | 보일러 시절 레거시 + **폐기된 Halo 킷**(`halo-design-kit/` — 2026-09-23 여기로 옮겼다). **사용자는 볼 필요 없음**, 에이전트만 배경 참조 |

> ⚠️ **디자인 토큰 원본은 `설계/` 안에 없다.** 현행 킷은 **저장소 루트 `design_kits_lets/`** 하나뿐이고,
> `frontend/src/main.jsx`가 거기서 직접 import한다. **`설계/` 안에 킷 사본을 만들지 마라** — 두 벌이 되면 조용히 갈린다.
