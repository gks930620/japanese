...
//
// ★ 2026-09-14: 계단이 사라지고 이 배열은 **사용자가 고르는 레벨 선택지**가 됐다(D9). 이름·구성·순서는 그대로다.
//
// 진단 레벨이 될 수 있는 코드(`STAGE_LEVEL_CODES`)는 프론트에 하드코딩돼 있고, 자료실의 레벨 선택지도
// 따로 하드코딩돼 있다. 둘은 **같은 사실의 두 사본**이라 갈리면 조용히 어긋난다 — 이 가드가 그것을 막는다.
//
// ★ 2026-09-10: 상대가 **한자 자료실 → 문법 자료실**로 바뀌었다.
//   예전 목록은 "한자가 있는 레벨"이었다(단계마다 한자 1문항). 개편으로 한자 낱자 문항이 빠지고
//   재료가 **어휘·문법**이 되면서, 목록의 기준은 "한자가 있는 레벨"이 아니라 **"코스가 있는 레벨 전부"** 다.
...
describe("진단 레벨 목록 — 두 하드코딩 사본은 같은 값이어야 한다 (M1 가드)", () => {
  ...
  /** 영어 과정 코드는 선택지에 들어가지 않는다 — 진단은 일본어 과정의 도구다(설계/09 §5) */
  it("영어 레벨 코드는 선택지가 되지 않는다", () => {
