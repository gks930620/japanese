...
export function LoginPage() {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      // 429는 눌러서 회복되지 않는다 — 얼마나 기다려야 하는지 말한다(08 C-12 ④)
      setError(errorText(e, "로그인에 실패했습니다."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="login-container">
      <div className="logo-section">
        ...
        {/* 보호된 화면에서 보내졌으면 **왜 여기 왔는지** 말한다 — 사용자가 누른 것이 왜 사라졌는지
            모르게 두지 않는다. 직접 온 사람에게는 없는 이유를 지어내지 않는다(2026-09 판정 D-11) */}
        <p className="logo-subtitle">
          {location.state?.from ? "로그인이 필요해요 — 로그인하면 보던 화면으로 돌아갑니다" : "서비스에 로그인하세요"}
        </p>
      </div>
